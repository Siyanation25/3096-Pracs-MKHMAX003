...
uint32_t pollADC(void){
  ...
  // while(1){
    ...
    // checking if adv conversion has succesfuly completed
    if(HAL_ADC_PollForConversion(&hadc, HAL_MAX_DELAY) == HAL_OK){

      // start reading the adc val
...
uint32_t ADCtoCCR(uint32_t adc_val){
  ...
  uint16_t ccr_val = (47999/4096)* adc_val;
