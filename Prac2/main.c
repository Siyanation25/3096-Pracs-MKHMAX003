...
int main(void)
{
  ...
  address_ = 0;
  for (int i=0;i<6;i++){
	 write_to_address(address_, patterns[i]);
	 address_ += 1;
  }
...
void TIM16_IRQHandler(void)
{
	...
	uint16_t address_read = address_%6;
	uint8_t read_binary = read_from_address(address_read);
	if (patterns[address_read] != read_binary){
		read_binary = 0b00000001;
	}
	GPIOB->ODR = read_binary;
	address_++;
}
